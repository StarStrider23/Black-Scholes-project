...
delta_errors, gamma_errors, vega_errors, theta_errors, rho_errors = [], [], [], [], []

for h in h_values:
    delta_num = model.compute_delta_num(h=h)
    gamma_num = model.compute_gamma_num(h=h)
    vega_num = model.compute_vega_num(h=h)
    theta_num = model.compute_theta_num(h=h)
    rho_num = model.compute_rho_num(h=h)

    delta_errors.append(abs(delta - delta_num))
    gamma_errors.append(abs(gamma - gamma_num))
    vega_errors.append(abs(vega - vega_num))
    theta_errors.append(abs(theta - theta_num))
    rho_errors.append(abs(rho - rho_num))
